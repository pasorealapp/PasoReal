...
(() => {
  ...
  function fillMembershipScreen() {
    ...
    if ($("memberName")) {
      const fullName = [p.nombre || "Usuario", p.apellido || ""].join(" ").trim();
      $("memberName").textContent = fullName || p.publicId || "Usuario";
    }
    ...
    const btnWhatsApp = $("btnSendProofWhatsApp");
    const btnEmail = $("btnSendProofEmail");
    const btnRefresh = $("btnRefreshMembership");
    const btnBack = $("btnBackFromMembership");
  ...
  function setupMembershipButtons() {
    $("btnSendProofWhatsApp")?.addEventListener("click", () => {
      ...
      const nombre = window.APP_STATE.profile?.nombre || "";
      const apellido = window.APP_STATE.profile?.apellido || "";

      const msg = encodeURIComponent(
        `Hola, envío comprobante de pago de PasoReal.\nNombre: ${nombre} ${apellido}\nUID: ${uid}\nCorreo: ${email}`
      );
    ...
    $("btnBackFromMembership")?.addEventListener("click", () => {
      updateHomeTexts();
      safeShow("home");
    });
